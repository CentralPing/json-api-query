...
import {JSON_API_PAGE_STRATEGY_GROUPS as fromWire} from '@centralping/ergo-wire';

import {JSON_API_PAGE_STRATEGY_GROUPS} from './index.js';
import schema from './schema.json' with {type: 'json'};

describe('[Module] JSON_API_PAGE_STRATEGY_GROUPS alignment', () => {
  it('re-exports the ergo-wire binding', () => {
    assert.equal(JSON_API_PAGE_STRATEGY_GROUPS, fromWire);
  });
  ...
  it('defines mutually exclusive pagination strategy groups', () => {
    ...
    assert.deepEqual(flat, [...new Set(flat)], 'strategy groups must be pairwise disjoint');
  });
});
